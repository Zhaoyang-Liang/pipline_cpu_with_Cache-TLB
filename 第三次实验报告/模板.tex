...
\subsection{AdEL 异常测试程序}

\textbf{测试目的：} 验证AdEL异常（异常码9）触发与恢复机制。
...
50H是异常处理地址，可以看到50H处，后面的指令在50H走到EXE阶段后，跳转为了00H（异常处理成功，也就是成功检测出了异常）：
...
同时设置Cause寄存器中的ExcCode位为和EPC为50H。操作系统/异常处理函数后续可以根据这两个寄存器的值做更详细的处理。
...
\textbf{程序说明：}  也差不多，只是后面的cause变了一下。这里直接附上图片吧：

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/break.png}
    \caption{BREAK异常测试程序}
\end{figure}


这个发生异常的地方是48H，由于这个是最早弄的，当时的testbench还没有把异常处理寄存器给引出来，所以当时没有最下面三个寄存器的值，不太直观，但可以看到后面两个周期也是成功跳转回了00H。\textbf{同时也可以看到4C和50H都被cancle掉了，没有成功WB（前面的所有异常都成功实现了这个）。}
