...
\vspace{1em}
\noindent\textbf{关键词：} 五级流水CPU设计图、TLB、I/DCache、github代码
...
D-cache和TLB我都用ad090000这个例子（一个store指令），可以先看图“
...
图中其实写的比较清楚，store后，tag变化，re\_is\_store变为1，然后开始写直达。最后tag变化就是写成功的证明
...
最后效果可以看到从ROM中拿到了这个指令。

那个图中等待TLB的意思准确来说是等待TLB、Dcache、AXI都处理完。
...
异常：可以看到badaddr、vaddr、i都是同样的0x00000004。（缺页异常（这里是往里面store，但效果一样））。
...
除了3.2 D-Cache 图中展示的那样（图中画出了级联流水线延迟），还有就是下面的一个
...
\section{实验总结}
本次实验在原有五级流水CPU上完成了TLB与I/D Cache的最简集成，并通过AXI双通道实现指令与数据的并行访问。通过在MEM阶段统一产生访存异常、在WB阶段统一仲裁上报CP0，保证了TLB异常与对齐异常的正确优先级与处理路径。

I-Cache采用只读直映结构，D-Cache采用写直达+写分配。仿真日志和仿真图都可以说明取指/访存命中路径、miss回填路径、写直达时的流水暂停以及TLB异常上报均按设计运行。



\paragraph{实验感想}

整体感觉良好，最后还是成功实现了，实现中现在习惯性在每个指令的关键点display出来一个信息，这样就不用费劲心思找那个仿真图了，感觉是个加快效率的方法。

回顾一学期的实验，我一方面自学了Vivado的一些更高级的功能，例如看CPU的能耗性能、加了约束后布线导致的问题可能需要调整频率等等（实验一复现CPU时候的事情）。从设计旁路等开始自己一步步设计，画设计图（包括这次的最大的那个CPU全局图），这个过程还是很有成就感的。

还有就是做AXI总线的时候最为困惑，因为不知道这个东西应该接到哪里，然后就查资料看b站的网课，最后成功实现。当然也看一部分CPU设计实战的书，但更多的时候我的设计夹杂了很多自己的理解和网上学的东西，肯定和书上还不完全一样感觉，但我也有参考。那个书写的挺全面也很启发思考（尤其是bypass那一章节），但是其余的有些章节过于臃肿写的，而且很杂，于是就十分困惑。

也学到了Vivado的Design手动绘制链接功能，当时也尝试过手动design AXi总线，还有就是很多东西也是基于vivado的IP核然后照着读照着理解写的代码，自己从头实现还是有些难。

总结就是最后实现成功的时候比较有成就感，然后能自己绘制出CPU设计图也比较有意思，但是有时候明明已经完全理解了还要把所有东西详细写成实验报告的时候比较折磨（享受）。

总之是更会写verilog更会用vivado了，也更深刻的理解了TLB和Cache还有AXI总线这些之前没接触过的知识。


\paragraph{实验建议}

有一个建议是做中断实验的时候，由于CPU的中断要和软件（异常处理程序）配合才能实现，但是没有标准的配套代码，那个时候就只能用大模型生成一段（因为没有编译器），感觉可以弄一个标准的配套代码，但是也是理解了很久才意识到应该再用大模型自己重写一个程序。
